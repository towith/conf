...
; DLL
hVirtualDesktopAccessor := DllCall("LoadLibrary", "Str", A_ScriptDir . "\lib\windows-desktop-switcher\VirtualDesktopAccessor.dll", "Ptr")
...
getSessionId()
{
    ...
    return process2SessionId(ProcessId)
}

process2SessionId(ProcessId){
    DllCall("ProcessIdToSessionId", "UInt", ProcessId, "UInt*", SessionId)
    if ErrorLevel {
        OutputDebug, Error getting session id: %ErrorLevel%
        return
    }
    ; OutputDebug, Current Session Id: %SessionId% %ProcessId%
    return SessionId
}
...
MoveWindowToDesktop(desktopNumber,winTitle:="A",switchAfter:=true) {
