...
{ 
    ...
    */
    ...
    public static var theword;
    public static var thewordpath;
	...
	public static RulesOption("Enable xdebug header")
	BindPref("fiddlerscript.rules.enableXdebugHeader")
	var m_xdebug:boolean=false;
        
    public static RulesOption("Enalbe access origin control")
    BindPref("fiddlerscript.rules.enableAccessOrigin") 
    var m_accessOrigin:boolean=false;
    ...
    static function OnBeforeRequest(oSession: Session) {
		...
		if(m_xdebug){
			if(oSession.uriContains("test.aaa.com")){
				oSession.oRequest.headers["Cookie"]=oSession.oRequest.headers['Cookie'] +  ';XDEBUG_SESSION=PHPSTORM;';
			}
		}
       
        if(oSession.uriContains("translate.google.cn/translate_tts")){
            var s_qs = (oSession.url + "?").split("?")[1];
            var querystring = HttpUtility.ParseQueryString(s_qs);
            theword=querystring.Get("q");
            thewordpath="D:\\z_my\\_envoc\\sound\\" + theword + ".mp3";
            
            
            var existed=File.Exists(thewordpath);
            
            if(existed){
                FiddlerApplication.Log.LogString(thewordpath);
                //FiddlerObject.alert(thewordpath); 
                oSession.utilCreateResponseAndBypassServer();
                oSession.responseCode = 201;
                oSession.LoadResponseFromFile(thewordpath);
...
*/
    ...
    static function OnBeforeResponse(oSession: Session) {
        ...
        if(m_accessOrigin){
            FiddlerApplication.Log.LogString("change access origin" + oSession.id);
